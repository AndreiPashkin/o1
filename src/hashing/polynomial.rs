...
pub const fn polynomial_const(value: &[u8], num_bits: u32, seed: &PolynomialSeed) -> u32 {
    ...
    if a == 0 || (a as u128) >= P {
        panic!(r#""seed[0]" must be in the range [1, P-1]"#);
    }

    if (b as u128) >= P {
        panic!(r#"Key seed value out of range [0, P-1]"#);
    }
    ...
    fn test_polynomial_const_equivalence() {
        ...
        for str_len in [0, 1, 4, 8, 16, 255, 256, 257, 512, 1024] {
            let non_const_family = |seed: u64, num_buckets: usize| {
                ...
                let mut seed_array = [0u64; 1 + 1 + 64 + 1 + 64 + 1];
                seed_array.fill_with(|| rng.random());
                seed_array[0] = 1 + seed_array[0];

                let poly_seed = PolynomialSeed::from(seed_array);

                (
                    Box::new(move |value: &String| {
                        polynomial(value.as_bytes(), num_bits, &poly_seed) as usize
            ...
            let const_family = |seed: u64, num_buckets: usize| {
                ...
                (
                    Box::new(move |value: &String| {
                        polynomial_const(value.as_bytes(), num_bits, &poly_seed) as usize
                    }) as Box<dyn Fn(&String) -> usize>,
                    num_buckets_for_bits(num_bits) as usize,
                )
            };
